# coding: utf-8
import sys
import os
import subprocess

# ===========================
# 【核心修复】类继承式隐藏黑框补丁
# ===========================
# 必须放在所有 import 之前 (除了 sys, os, subprocess)
# 修复原理：使用 Class 继承而不是 Function 替换，解决 asyncio 报错
if sys.platform == "win32":
    # 1. 保存原版 Popen 类
    _OriginalPopen = subprocess.Popen


    # 2. 定义一个继承自原版 Popen 的新类
    class _HiddenPopen(_OriginalPopen):
        def __init__(self, *args, **kwargs):
            # A. 获取或创建 startupinfo
            startupinfo = kwargs.get('startupinfo')
            if startupinfo is None:
                startupinfo = subprocess.STARTUPINFO()

            # B. 强制注入隐藏属性
            # STARTF_USESHOWWINDOW = 0x00000001
            startupinfo.dwFlags |= subprocess.STARTF_USESHOWWINDOW
            # SW_HIDE = 0
            startupinfo.wShowWindow = subprocess.SW_HIDE

            kwargs['startupinfo'] = startupinfo

            # C. 强制注入 CREATE_NO_WINDOW 标志
            # 防止某些情况下 cmd 窗口一闪而过
            creationflags = kwargs.get('creationflags', 0)
            # 0x08000000 是 CREATE_NO_WINDOW
            kwargs['creationflags'] = creationflags | 0x08000000

            # D. 调用父类(原版)的初始化，维持 asyncio 的兼容性
            super().__init__(*args, **kwargs)


    # 3. 用新类覆盖原版类
    subprocess.Popen = _HiddenPopen
# ===========================

import asyncio
import signal
from pathlib import Path
from platform import system
from typing import List
import typer
import colorama

# 路径修正：确保能读取同级目录的 config.py
if getattr(sys, 'frozen', False):
    BASE_DIR = os.path.dirname(sys.executable)
    sys.path.insert(0, BASE_DIR)
else:
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
os.chdir(BASE_DIR)

# 导入业务模块
from config import ClientConfig as Config
from util.client_cosmic import console, Cosmic
from util.client_stream import stream_open, stream_close
from util.client_shortcut_handler import bond_shortcut
from util.client_recv_result import recv_result
from util.client_show_tips import show_mic_tips, show_file_tips
from util.client_hot_update import update_hot_all, observe_hot
from util.client_transcribe import transcribe_check, transcribe_send, transcribe_recv
from util.client_adjust_srt import adjust_srt
from util.empty_working_set import empty_current_working_set

# 确保终端能使用 ANSI 控制字符
colorama.init()

# MacOS 的权限设置
if system() == 'Darwin' and not sys.argv[1:]:
    if os.getuid() != 0:
        print('在 MacOS 上需要以管理员启动客户端才能监听键盘活动，请 sudo 启动')
        input('按回车退出');
        sys.exit()
    else:
        os.umask(0o000)


async def main_mic():
    Cosmic.loop = asyncio.get_event_loop()
    Cosmic.queue_in = asyncio.Queue()
    Cosmic.queue_out = asyncio.Queue()

    # 此处虽然叫 show_tips，但实际上输出会被 GUI 捕获
    # show_mic_tips()

    # 更新热词
    update_hot_all()

    # 实时更新热词
    observer = observe_hot()

    # 打开音频流
    Cosmic.stream = stream_open()

    # Ctrl-C 关闭音频流
    signal.signal(signal.SIGINT, stream_close)

    # 绑定按键
    bond_shortcut()

    # 清空物理内存工作集
    if system() == 'Windows':
        empty_current_working_set()

    # 接收结果
    while True:
        await recv_result()


async def main_file(files: List[Path]):
    show_file_tips()

    for file in files:
        if file.suffix in ['.txt', '.json', 'srt']:
            adjust_srt(file)
        else:
            await transcribe_check(file)
            await asyncio.gather(
                transcribe_send(file),
                transcribe_recv(file)
            )

    if Cosmic.websocket:
        await Cosmic.websocket.close()
    input('\n按回车退出\n')


def init_mic():
    try:
        asyncio.run(main_mic())
    except KeyboardInterrupt:
        console.print(f'再见！')
    finally:
        pass  # 静默退出


def init_file(files: List[Path]):
    try:
        asyncio.run(main_file(files))
    except KeyboardInterrupt:
        console.print(f'再见！')
        sys.exit()


if __name__ == "__main__":
    if sys.argv[1:]:
        typer.run(init_file)
    else:
        init_mic()